...
These examples build on the setup from [Create an NG-CHM (Getting Started)](create-an-ng-chm.html).
...
colOrder <- colnames(matirx_data)[sample.int(length(colnames(matrix_data)))]
...
ordering. The example below uses the clustering results from above as dedrograms:
